...
export function buildStage2Prompt({ goal, stage1Json, refImages }) {
  ...
  const user = [
    ...
    `- Have class=\"target\" + onclick=\"onTargetClick(N)\" where N is the step number (1,2,3...${stepCount})`,
    ...
    "  if(window.TargetBridge)window.TargetBridge.postMessage(JSON.stringify({event:'target_click',stepIndex:step||(currentPage+1)}));",
...
export function buildChatGeneratePrompt({ goal, stepCount }) {
  ...
  const user = [
    ...
    "Target: a native-looking app element with class=\"target\" and onclick=\"onTargetClick(N)\" (N=1,2,3... matching the step).",
    "onTargetClick(N) notifies Flutter then switches to the next page; on last step, shows completion.",
    "",
    "=== JS (include exactly) ===",
